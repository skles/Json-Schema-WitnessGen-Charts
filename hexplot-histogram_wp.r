...
hexplot_fix <- function(col.low="#132B43", col.high="#56B1F7", col.med="red", col.inf="red", y_title="") {
  ...
  plot <- ggplot(data=df, aes(x = inSize, y=totalTime)) + 
    ...
    geom_text(aes(x=size_median, label="Median", y=10^3.5), nudge_x=-0.2, nudge_y=-0.2, colour=col.med, 
    ...
    geom_text(aes(x=10^3.5, label="Median", y=time_median), nudge_x=0.1, nudge_y=-0.5, colour=col.med, 
